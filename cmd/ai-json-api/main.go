...
func main() {
	...
	if pollSeconds > 0 {
		go runPeriodicIngestion(s, streamPath, time.Duration(pollSeconds)*time.Second, minAge)
	}
	...
	fmt.Fprintf(os.Stdout, "stream=%s poll_seconds=%d min_file_age_seconds=%d\n", streamPath, pollSeconds, minFileAgeSecond)
	if err := srv.ListenAndServe(); err != nil && err != http.ErrServerClosed {
		fatalf("listen: %v", err)
	}
}

func runPeriodicIngestion(st *store.Store, streamPath string, interval time.Duration, minFileAge time.Duration) {
	runner := ingest.Runner{Store: st, StreamPath: streamPath, MinFileAge: minFileAge}
